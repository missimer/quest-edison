...
#define DEBUG_INTEL_SCU_FLIS

#ifdef DEBUG_INTEL_SCU_FLIS
#define DLOG(fmt,...) DLOG_PREFIX("intel-scu-flis",fmt,##__VA_ARGS__)
#else
#define DLOG(fmt,...) ;
#endif
...
#define dev_err(y, x, ...) DLOG(x, ##__VA_ARGS__)
...
void set_flis_value(u32 value, u32 offset)
{
   ...
   */
  if (offset >= 0x1d00 && offset <= 0x1d34) {
    DLOG("Case in %s not implemented");
    panic("Case in set_flis_value not implemented");
...
scu_flis_probe(void)
{
  ...
  int ret;
  struct intel_scu_flis_info *isfi = &flis_info;
  struct intel_scu_flis_platform_data *pdata = &flis_pdata;
  ...
  if (pdata->mmio_flis_t && pdata->flis_base) {
    ...
    isfi->flis_base = map_contiguous_virtual_pages(pdata->flis_base | 0x1B,
                                                   pdata->flis_len / 0x1000);
    if (!isfi->flis_base) {
